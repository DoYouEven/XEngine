...
bool Collider::raysphere(float xc, float yc, float zc, float xd, float yd, float zd, float xs, float ys, float zs, float r, float* dist, vec3* collpoint)
{
...
float Collider::trianglearea(vec3 p1, vec3 p2, vec3 p3)
{
...
float Collider::pointdistace(vec3 p1, vec3 p2)
{
	vec3 vec(p2.x - p1.x, p2.y - p1.y, p2.z - p1.z);
	return (sqrt(vec.x*vec.x + vec.y*vec.y + vec.z*vec.z));
}

float Collider::pointdistacesquare(vec3 p1, vec3 p2)
{
	vec3 vec(p2.x - p1.x, p2.y - p1.y, p2.z - p1.z);
...
bool Collider::rayplane(const float& nx, float ny, float nz, float x0, float y0, float z0, float xs, float ys, float zs, float xd, float yd, float zd, vec3 p1, vec3 p2, vec3 p3, vec3 p4, float* dis, vec3* point)
{
	...
	float x = xs + t*xd;	//calculate the 3 point vec3
	float y = ys + t*yd;
	float z = zs + t*zd;
	vec3 i(x, y, z);
...
bool Collider::sphereplane(vec3& sp, vec3 pn, vec3 p1, vec3 p2, vec3 p3, vec3 p4, float r)
{
	...
	if (rayplane(pn.x, pn.y, pn.z, p1.x, p1.y, p1.z, sp.x, sp.y, sp.z, -pn.x, -pn.y, -pn.z, p1, p2, p3, p4, &dist1) || rayplane(-pn.x, -pn.y, -pn.z, p1.x, p1.y, p1.z, sp.x, sp.y, sp.z, pn.x, pn.y, pn.z, p1, p2, p3, p4, &dist2))
	{
		...
		return true;	//we return true, to indicate the Collider, but the main thing is, that we changged the sp (so the sphere position) to the new position)
	}
...
bool Collider::spheresphere(vec3& c1, float r1, vec3& c2, float r2)
{
