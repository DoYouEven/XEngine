...
void drawCube()
{
	...
	//glTranslatef(-MainCamera.getPosition().x, 5, -MainCamera.getPosition().z);

	//glRotatef(-MainCamera.getRotX(), 1, 0, 1);
	//glRotatef(-MainCamera.getRotY(), 0, 1, 0);
	glBegin(GL_QUADS);
...
void drawGrid()
{
	...
	float incrX = width / (float)divisions;
	float incrY = height / (float)divisions;
	//glDisable(GL_TEXTURE_2D);
	glColor3f(1, 1, 1);
	glNormal3f(0, 1, 0);
	for (float i = -width / 2; i < width / 2; i += incrX)
	{
		for (float j = -height / 2; j < height / 2; j += incrY)
		{
			glBegin(GL_TRIANGLE_STRIP);
			glVertex3f(i + incrX, 0, j + incrY);
			glVertex3f(i, 0, j + incrY);
			glVertex3f(i + incrX, 0, j);
			glVertex3f(i, 0, j);
			glEnd();
		}
...
GLvoid drawCubes()
{
	if (gridList == 0)
	{
		...
		for (GLfloat x = -20.f; x < 20.f; x += 2.5f)
		{
				...
				if (mesh && mesh->getParent() == NULL)
				{
					if (item_size < 1)
						...
						for (list<Mesh*>::iterator m = itms.begin(); m != itms.end(); m++)
						{
							...
							if (mesh->drawMesh(true, inMesh,true))
							{
								//XUtils->draw_axes();
								item_size -= 1;
...
int main(int argc, char**argv)
{
	...
	btnSpinLights = (iButton*)XUtils->addInspectorTopFrame(new iButton("Toggle Spin Lights", 0, 0, BUTTON_WIDTH + 30, BUTTON_HEIGHT, UITextures[BUTTON_BASE], UITextures[BUTTON_HOVER]));
	...
	addThirdPersonController = (iButton*)XUtils->addInspectorTopFrame(new iButton("Add third person controller", 0, 0, BUTTON_WIDTH+40, BUTTON_HEIGHT, UITextures[BUTTON_BASE], UITextures[BUTTON_HOVER]));
	...
	MainLight->setDiffuse(0.3, 0.35, 0.32, 1.0f);
	MainLight->setAmbient(1.0, 1.0, 1.0, 1.0f);
	MainLight->setPosition(0, 60, 0);


	//*************************SLIDERS*******************************************************
	STX = (iSlider*)XUtils->addInspector(new iSlider("Translate X", -100, 100, 0, 0, 250, 40, UITextures[SLIDER_BASE], UITextures[SLIDER_KNOB], UITextures[INS_BG]));
	STY = (iSlider*)XUtils->addInspector(new iSlider("Translate Y", -100, 100, 0, 0, 250, 40, UITextures[SLIDER_BASE], UITextures[SLIDER_KNOB], UITextures[INS_BG]));
	STZ = (iSlider*)XUtils->addInspector(new iSlider("Translate Z", -100, 100, 0, 0, 250, 40, UITextures[SLIDER_BASE], UITextures[SLIDER_KNOB], UITextures[INS_BG]));

	SRX = (iSlider*)XUtils->addInspector(new iSlider("Rotation X", -360, 360, 0, 0, 250, 40, UITextures[SLIDER_BASE], UITextures[SLIDER_KNOB], UITextures[INS_BG]));
	SRY = (iSlider*)XUtils->addInspector(new iSlider("Rotation Y", -360, 360, 0, 0, 250, 40, UITextures[SLIDER_BASE], UITextures[SLIDER_KNOB], UITextures[INS_BG]));
	SRZ = (iSlider*)XUtils->addInspector(new iSlider("Rotation Z", -360, 360, 0, 0, 250, 40, UITextures[SLIDER_BASE], UITextures[SLIDER_KNOB], UITextures[INS_BG]));

	SSX = (iSlider*)XUtils->addInspector(new iSlider("Scale X", .1, 10, 0, 0, 250, 40, UITextures[SLIDER_BASE], UITextures[SLIDER_KNOB], UITextures[INS_BG]));
	SSY = (iSlider*)XUtils->addInspector(new iSlider("Scale Y", .1, 10, 0, 0, 250, 40, UITextures[SLIDER_BASE], UITextures[SLIDER_KNOB], UITextures[INS_BG]));
	SSZ = (iSlider*)XUtils->addInspector(new iSlider("Scale Z", .1, 10, 0, 0, 250, 40, UITextures[SLIDER_BASE], UITextures[SLIDER_KNOB], UITextures[INS_BG]));
